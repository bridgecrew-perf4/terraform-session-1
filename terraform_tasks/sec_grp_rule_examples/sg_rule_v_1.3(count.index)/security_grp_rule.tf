# In this example of sg_rule we used count meta-argument with index and length functions,
# where we were able to provision 2 ingress rules (ssh and http) and egress rule. The 
# reason for putting an extra [] signs for cidr blocks is because it's a list, which 
# requires to be inside of the []. Another thing to mention you can either keep the list
# inside of the variables.tf or dev.tf, either way it will work.
