# In this version 2 vpc template, "element" and "length" functions were used, 
# with one subnet resource block 3 subnets will created, also on route table association
# we used the same functions. For tags we used locals in order to avoid repeatition of 
# the same tags. Added outputs file which will output subnets ids and cidr blocks,
# and vpc id.
